...
from gaussianshopvr.utils.general_utils import (
    inverse_sigmoid,
    get_expon_lr_func,
    build_rotation,
)
...
from gaussianshopvr.utils.sh_utils import RGB2SH, SH2RGB
...
from gaussianshopvr.core.gaussian_renderer import camera2rasterizer
from gaussianshopvr.utils.knn import K_nearest_neighbors
...
class SimpleGaussianModel:
    ...
    def __init__(self, sh_degree: int = 0):
        ...
        self._xyz = torch.empty((0, 3)).cuda().requires_grad_(True)
        self._features_dc = torch.empty(0, 1, 3).cuda().requires_grad_(True)
        self._features_rest = (
            torch.empty(0, (self.max_sh_degree + 1) ** 2 - 1, 3)
            .cuda()
            .requires_grad_(True)
        )
        self._opacity = torch.empty(0, 1).cuda().requires_grad_(True)
        self._scaling = torch.empty(0, 3).cuda().requires_grad_(True)
        self._rotation = torch.empty(0, 4).cuda().requires_grad_(True)
        self._id = torch.empty(0, dtype=torch.int).cuda()
        self.mask = torch.zeros(0, dtype=torch.bool).cuda()

        self.setup_functions()

    @property
    def points_num(self):
        return self._opacity.size()[0]
    ...
    @property
    def get_rgb(self):
        return SH2RGB(self._features_dc.squeeze()).clamp(0, 1)
    ...
    def construct_list_of_attributes(self):
        ...
        for i in range(15 * self._features_rest.shape[2]):
            l.append("f_rest_{}".format(i))
        l.append("opacity")
    ...
    def save_ply(self, path, mask=None):
        ...
        if mask is None:
            mask = torch.ones(self._opacity.size()[0], dtype=torch.bool)
        ...
        f_rest = (
            torch.zeros((self._xyz.shape[0], 15, 3))
            .detach()
            .transpose(1, 2)
            .flatten(start_dim=1)
            .contiguous()
...
class GaussianModel(SimpleGaussianModel):
    def __init__(
        self,
        sh_degree: int = 3,
    ):
    ...
    def generation_num(self):
        return len(getattr(self, "_anchor_loss_schedule", []))
    ...
    def anchor_loss(self):
        ...
        for key, value in self.anchor.items():
            ...
            if key in ["_xyz"]:
                out["loss_anchor_pos"] += delta
            elif key in ["_features_dc"]:
    ...
    def create_from_pcd(self, pcd: BasicPointCloud, spatial_lr_scale: float):
        ...
        self.mask = torch.zeros(self.points_num, dtype=torch.bool, device="cuda")
        self._id = torch.zeros(self.points_num, device="cuda", dtype=torch.int)
    ...
    def set_opacity(self, new_value, mask=None):
        if mask is None:
            mask = torch.ones(self.points_num).bool()
        new_value = inverse_sigmoid(new_value.detach().clone().float().cuda())
        new_opacities = self._opacity.detach().clone()
        new_opacities[mask] = new_value[mask].unsqueeze(1)
        optimizable_tensors = self.replace_tensor_to_optimizer(new_opacities, "opacity")
        self._opacity = optimizable_tensors["opacity"]

        self.remove_grad_mask()
        self.apply_grad_mask()

    def set_rgb(self, new_value, mask=None):
        if mask is None:
            mask = torch.ones(self.points_num).bool()
        fused_color = RGB2SH(new_value.detach().clone().float().cuda())
        new_features_dc = self._features_dc.detach().clone()
        new_features_dc[mask, 0, :3] = fused_color[mask]
    ...
    def load_ply(self, path):
        ...
        # print(len(extra_f_names), 3 * (self.max_sh_degree + 1) ** 2 - 3)
        ...
        # set all points to the root group at first
        self._id = torch.zeros(self.points_num, dtype=torch.int, device="cuda")
        self.mask = torch.zeros(self.points_num, dtype=torch.bool, device="cuda")
    ...
    def densification_postfix(
        ...
        if new_id is not None:
            ...
            self._id = torch.full(
                (self._xyz.shape[0],), self._id[0], dtype=torch.int
            ).cuda()

        self.xyz_gradient_accum = torch.zeros((self.points_num, 1), device="cuda")
        self.denom = torch.zeros((self.points_num, 1), device="cuda")
        self.max_radii2D = torch.zeros((self.points_num), device="cuda")
    ...
    def apply_grad_mask(self):
        ...
        for field in fields:
            ...
            assert (
                this_field.is_leaf and this_field.requires_grad
            ), f"{field}, is_leaf={this_field.is_leaf}, requires_grad={this_field.requires_grad}"
    ...
    def concat_gaussians(
        ...
        ts = pytorch3d.transforms.Scale(*scale).cuda()
        # pytorch3d uses row major ordering, but quaterion_to_matrix get col major matrix
        tr = pytorch3d.transforms.Rotate(
            pytorch3d.transforms.quaternion_to_matrix(torch.tensor(wxyz)).transpose(
                -1, -2
            )
        ).cuda()
        tt = pytorch3d.transforms.Translate(*translation).cuda()

        new_xyz = ts.compose(tr, tt).transform_points(another_gaussian._xyz)
        new_scaling = another_gaussian._scaling + torch.log(torch.tensor(scale).cuda())

        new_rotation = pytorch3d.transforms.quaternion_multiply(
            torch.tensor(wxyz).cuda(),
        ...
        self.mask = torch.zeros(self.points_num, dtype=torch.bool).cuda()
        self.remove_grad_mask()
        self.apply_grad_mask()
        ...
        mask = torch.zeros((self.points_num), dtype=torch.bool)
        mask[-new_xyz.shape[0] :] = True
        return mask
