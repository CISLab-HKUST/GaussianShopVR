...
public class SaveModelTool : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            SaveActivatedModels();
            ReminderManager.Instance.ShowReminder("Activated Models Saved.");
        }
    }

    public void SaveActivatedModels()
    {
        List<GameObject> activatedGSs = modelManager.GetActiveModels();
        for (int i = 0; i < activatedGSs.Count; i++)
        {
            serverSyncer.SaveModel(activatedGSs[i].GetComponent<ObjInfo>().remoteInfo.id);
        }
    }
