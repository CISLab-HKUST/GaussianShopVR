...
public class GaussianSplattingCamera : MonoBehaviour
{
    ...
    protected GameObject real_leye,
        real_reye;
    ...
    protected void OnPreRender()
    {
        ...
        if (
            ...
            || renderMaterial == null
            || (
                gs.state != GaussianSplatting.State.RENDERING
                && gs.state != GaussianSplatting.State.PAUSE
            )
        )
        {
            return;
        ...
        if (isXr)
        {
            ...
            )
            {
                if (real_leye == null)
                {
                    real_leye = new GameObject("real leye");
                    real_leye.transform.parent = cam.transform.parent;
                }
                ...
                if (real_reye == null)
                {
                    real_reye = new GameObject("real reye");
                    real_reye.transform.parent = cam.transform.parent;
                }
                ...
                if (tex != null && tex[i] != null)
                {
                    ...
                    if (isXr)
                    {
                        if (i == 0)
                        {
                            ...
                            // proj_mat = Matrix4x4.Perspective(fovy * Mathf.Rad2Deg, aspect, near, far);

                            // proj_mat[0, 2] = -0.242510f;
                            // proj_mat[1, 2] = -0.19319f;
                            // Debug.Log(proj_mat);
    ...
    protected void OnRenderImage(RenderTexture source, RenderTexture destination)
    {
        ...
        for (int i = 0; i < (isXr ? 2 : 1); ++i)
        {
            renderMaterial.SetTexture(
                i == 0 ? "_GaussianSplattingTexLeftEye" : "_GaussianSplattingTexRightEye",
                tex[i]
            );
            renderMaterial.SetTexture(
                i == 0 ? "_GaussianSplattingDepthTexLeftEye" : "_GaussianSplattingDepthTexRightEye",
                depthTex[i]
            );
        }
    ...
    protected IEnumerator Initialize()
    {
        ...
        {
            return gs.state == GaussianSplatting.State.RENDERING;
        });
        ...
        Vector2Int resolution = new Vector2Int(
            (int)(cam.pixelWidth * texFactor),
            (int)(cam.pixelHeight * texFactor)
        );
        InternalTexSize = resolution;
        int[] temp_pov = { 0, 0 };
        for (int i = 0; i < (isXr ? 2 : 1); ++i)
        {
            camDepthRTex[i] = new RenderTexture(
                resolution.x,
                resolution.y,
                0,
    ...
    protected static bool TryGetEyesPoses(
        out Vector3 lpos,
        out Vector3 rpos,
        out Quaternion lrot,
        out Quaternion rrot
    )
    {
        ...
        foreach (XRNodeState state in states)
        {
            ...
            {
                if (state.TryGetPosition(out Vector3 tpos))
                {
                    lpos = tpos;
                    nbfound += 1;
                }
                if (state.TryGetRotation(out Quaternion trot))
                {
                    lrot = trot;
                    nbfound += 1;
                }
