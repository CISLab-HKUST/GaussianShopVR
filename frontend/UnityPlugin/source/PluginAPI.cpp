...
bool PluginAPI::LoadModel(const char *file)
{
	if (file == nullptr)
	{
		_message = "filepath cannot be null";
		return false;
	}
	try
	{
...
void PluginAPI::SetEditParameters(int model, float *position, float *rotation, float *scale)
{
	// Quaternionf q = Quaternionf(Eigen::AngleAxisf(M_PI, Vector3f::UnitZ())) * Quaternionf(rotation[3], rotation[0], -rotation[1], -rotation[2]);
	Quaternionf q = Quaternionf(rotation[3], rotation[0], rotation[1], rotation[2]);
	Eigen::Matrix<float, 3, 3, 0, 3, 3> s = q.toRotationMatrix();
	Matrix4f rotmat;
	rotmat << s(0, 0), s(0, 1), s(0, 2), 0,
		s(1, 0), s(1, 1), s(1, 2), 0,
		s(2, 0), s(2, 1), s(2, 2), 0,
		0, 0, 0, 1;
	...
	Matrix4f model_mat = transform * scalemat;
	splat.SetEditParameters(model, model_mat);
}
...
{
	// Edit first,do not need run by two povs
	for (auto kv : povs)
	{
...
void PluginAPI::Draw()
{
	...
	for (auto kv : povs)
	{
		...
		if (pov->is_initialized && pov->is_drawable && pov->is_preprocessed && !pov->is_drawn)
		{
			...
			{
				// Map image
				cudaGraphicsMapResources(1, &pov->imageBufferCuda);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cudaGraphicsSubResourceGetMappedArray(&(m_mapped_array[contextidx]), pov->imageBufferCuda, 0, 0);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				cudaCreateSurfaceObject(&(m_surface[contextidx]), &resource_desc);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				// Map depth
				cudaGraphicsMapResources(1, &pov->imageDepthBufferCuda);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cudaGraphicsSubResourceGetMappedArray(&(m_depth_mapped_array[contextidx]), pov->imageDepthBufferCuda, 0, 0);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				cudaCreateSurfaceObject(&(m_depth_surface[contextidx]), &depth_resource_desc);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				// Map camera depth
				cudaGraphicsMapResources(1, &pov->imageCameraDepthBufferCuda);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cudaGraphicsSubResourceGetMappedArray(&(m_camera_depth_mapped_array[contextidx]), pov->imageCameraDepthBufferCuda, 0, 0);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				cudaCreateSurfaceObject(&(m_camera_depth_surface[contextidx]), &camera_depth_resource_desc);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
			...
			{
				// Copy image & depth
				cuda_splat_to_texture(pov->width, pov->height, 4, image_cuda, m_surface[contextidx]);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cuda_splat_to_texture(pov->width, pov->height, 1, depth_cuda, m_depth_surface[contextidx]);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
			...
			if (!pov->_interop_failed)
			{
				cudaDestroySurfaceObject(m_surface[contextidx]);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cudaGraphicsUnmapResources(1, &pov->imageBufferCuda);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				cudaDestroySurfaceObject(m_depth_surface[contextidx]);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cudaGraphicsUnmapResources(1, &pov->imageDepthBufferCuda);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				...
				cudaDestroySurfaceObject(m_camera_depth_surface[contextidx]);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
				}
				cudaGraphicsUnmapResources(1, &pov->imageCameraDepthBufferCuda);
				if (CUDA_ERROR(_message))
				{
					pov->is_drawn = false;
					continue;
...
void POV::FreeCudaRessources()
{
	if (splatBufferCuda)
	{
		cudaFree(splatBufferCuda);
		splatBufferCuda = nullptr;
	}
	if (splatDepthBufferCuda)
	{
		cudaFree(splatDepthBufferCuda);
		splatDepthBufferCuda = nullptr;
	}
	if (imageBufferCuda)
	{
	...
	if (imageDepthBufferCuda)
	{
		cudaGraphicsUnregisterResource(imageDepthBufferCuda);
		imageDepthBufferCuda = nullptr;
	}
	...
	if (fallbackDepthBufferCuda)
	{
		cudaFree(&fallbackDepthBufferCuda);
		fallbackDepthBufferCuda = nullptr;
	}
}
...
bool POV::AllocSplatBuffer(std::string &message)
{
	...
	cudaMalloc(&splatBufferCuda, width * height * 4 * sizeof(float));
	if (CUDA_ERROR(message))
	{
		return false;
	}
	cudaMalloc(&splatDepthBufferCuda, width * height * 1 * sizeof(float));
	if (CUDA_ERROR(message))
	{
		return false;
	}
