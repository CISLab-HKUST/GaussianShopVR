...
__global__ void preprocessCUDA(int P, int D, int M,
	...
	float* boxmin,
	float* boxmax,
	const float* selectionCenter,
	const float selectionRadius,
	float* isSelected,
	const float* model_mat,
	bool isSelecting,
	bool isEraseSelection)
{
	...
	const float* model_matrix = &(model_mat[midx * 16]);
...
// cacalute point selection

	if (selectionCenter != nullptr && selectionRadius > 0.0f && isSelecting)
	{
		
		float4 world_points = transformPoint4x4(p_orig, model_matrix);
		if(isEraseSelection){
			isSelected[idx] = isSelected[idx] && (!point_in_sphere(idx,world_points, selectionCenter, selectionRadius));
		}else{
			isSelected[idx] = isSelected[idx] || point_in_sphere(idx,world_points, selectionCenter, selectionRadius);
		}
	}
...
__global__ void renderCenterOverlay(
    float2* means2D,      
    const float* depths,        
    float* out_color,          
    const int width,
	...
	const float *unselectedColor,
	float* isSelected,
	const float pointSize,
    const float depthcutoff)
{
	int idx = blockIdx.x * blockDim.x + threadIdx.x;
	if(depths[idx] > depthcutoff){
		return;
	}
    float2 center = means2D[idx];
	atomicExch(&means2D[idx].x, 0.0f);
    atomicExch(&means2D[idx].y, 0.0f);
    int centerX = (int)center.x;
    int centerY = (int)center.y;

	int radius = (int)(pointSize / 2.0f);
	for(int dy = -radius; dy <= radius; dy++) {
		for(int dx = -radius; dx <= radius; dx++) {
			...
			{
				if(isSelected[idx] == 1){
					atomicExch(&out_color[0 * width * height + pixel_idx], selectedColor[0]);
					atomicExch(&out_color[1 * width * height + pixel_idx], selectedColor[1]);
					atomicExch(&out_color[2 * width * height + pixel_idx], selectedColor[2]);
					atomicExch(&out_color[3 * width * height + pixel_idx], selectedColor[3]);
				}else if(isSelected[idx] == 0){
					atomicExch(&out_color[0 * width * height + pixel_idx], unselectedColor[0]);
					atomicExch(&out_color[1 * width * height + pixel_idx], unselectedColor[1]);
					atomicExch(&out_color[2 * width * height + pixel_idx], unselectedColor[2]);
					atomicExch(&out_color[3 * width * height + pixel_idx], unselectedColor[3]);
				}
			}
			else
			{
				atomicExch(&out_color[0 * width * height + pixel_idx], 0.0);
				atomicExch(&out_color[1 * width * height + pixel_idx], 1.0);
				atomicExch(&out_color[2 * width * height + pixel_idx], 0.0);
				atomicExch(&out_color[3 * width * height + pixel_idx], 1.0);
			}
		}
	}
}
	...
	const float depthcutoff)
{
	...
	if (show_centers){
		renderCenterOverlay<<<(P + 255) / 256, 256>>>(
			means2D,
			depth,
			out_color,
			W,
