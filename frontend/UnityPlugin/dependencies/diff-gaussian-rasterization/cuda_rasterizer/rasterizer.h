...
namespace CudaRasterizer
{
	...
	public:
		...
		static int forward_preprocess(
			...
			float *boxmin = nullptr,
			float *boxmax = nullptr,
			const float *selectionCenter = nullptr,
			const float selectionRadius = 0.0f,
			float *isSelected = nullptr,
			const float *model_mat = nullptr,
		...
		static void forward_render(
			...
			const float *unselectedColor = nullptr,
			bool show_centers = false,
			float *isSelected = nullptr,
			const float pointSize = 0.0f,
			const float depthcutoff = 0.0f);
